...
import com.izforge.izpack.panels.installationtype.InstallationTypeConsolePanel;
...
public class DataCheckConsolePanelTest
{
    ...
    {
        installData.setVariable("MyVar", "MyValue");
        ...
        DataCheckConsolePanel dccPanel = new DataCheckConsolePanel(panelView, installData);
        console.addScript("DataCheckPanel.1", "1");
        List<String> output = console.getOutput();
        assertTrue(dccPanel.run(installData, console));
        assertEquals("Debugging InstallData. All InstallData variables and all packs (selected packs are marked).", output.get(4));
        assertTrue(output.get(5).contains("MyVar"));
        assertTrue(output.get(5).contains("MyValue"));
        assertTrue(output.get(6).contains("Pack1(Selected)"));
        assertTrue(output.get(6).contains("Pack2(Unselected)"));
    }
